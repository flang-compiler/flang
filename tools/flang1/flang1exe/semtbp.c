...
clearStaleRecs(tbpTask task)
{
  ...
  if (IS_CLEAR_STALE_RECORDS_TASK(task)) {
    for (prev = curr = tbpQueue; curr; ) {
      if (curr->dtype >= (int)stb.dt.stg_avail) {
        ...
        if (curr->dtPass >= (int)stb.dt.stg_avail) {
          curr->dtPass = DT_NONE;
          ++numUpdated;
        }
        if (curr->impSptr >= (int)stb.stg_avail) {
          curr->impSptr = 0;
          ++numUpdated;
        }
        if (curr->bindSptr >= (int)stb.stg_avail) {
          curr->bindSptr = 0;
          ++numUpdated;
        }
        if (curr->memSptr >= (int)stb.stg_avail) {
          curr->memSptr = 0;
          ++numUpdated;
        }
        prev = curr;
