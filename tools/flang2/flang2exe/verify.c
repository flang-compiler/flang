...
is_known_bug(ILI_OP opc, int j, ILI_OP j_opc)
{
  ...
  if (opc == IL_STKR && o == ILIO_KRLNK && (r == ILIA_IR || r == ILIA_AR) &&
      j == 1)
    return true;
  if (opc == IL_KADD && o == ILIO_KRLNK && r == ILIA_AR)
    return true;
  if (opc == IL_ISUB && o == ILIO_IRLNK && r == ILIA_AR)
    return true;
  if (opc == IL_IKMV && o == ILIO_IRLNK && r == ILIA_AR)
    return true;
  ...
  if (opc == IL_STKR && o == ILIO_KRLNK && r == ILIA_IR && j == 1)
    return true;
  if (opc == IL_IADD && o == ILIO_IRLNK && r == ILIA_AR)
    return true;
  ...
  if (opc == IL_UKNEG && o == ILIO_KRLNK && r == ILIA_IR && j == 1)
    return true; 
  if (opc == IL_INEG && o == ILIO_IRLNK && r == ILIA_KR && j == 1)
    return true;
  ...
  if ((opc == IL_ST || opc == IL_ARGIR) && o == ILIO_IRLNK && j_opc == IL_KCON)
    return true;
  if (opc == IL_STKR && o == ILIO_KRLNK && j_opc == IL_ICON)
    return true;
  ...
  if ((opc == IL_IADD || opc == IL_IAMV) && o == ILIO_IRLNK && j_opc==IL_DFRKR)
    return true;
  if (opc == IL_DAAR && o == ILIO_ARLNK && j_opc == IL_LDKR)
    return true;
