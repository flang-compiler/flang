...
addbran(ILI *ilip)
{
  ...
  case IL_LCJMPZ:
     ...
     */
    assert(op2 == CC_EQ || op2 == CC_NE, "addbran:bad stc of LCJMPZ", op2,
           ERR_Severe);
    switch (ILI_OPC(op1)) {
...
/* generate sequence of AND and ICJMPZ ili */
  ...
  case IL_ICJMPZ:
    ...
    case IL_ICMPZ:
      if ((new_cond = combine_int_ccs(CC_ILI_OPND(op1, 2), (CC_RELATION)op2)) ==
          0) // ???
        break;
    ...
    case IL_ICMP:
      if ((new_cond = combine_int_ccs(CC_ILI_OPND(op1, 3), (CC_RELATION)op2)) ==
          0)
        break;
        ...
        if (need_to_rename) {
          if ((IL_TYPE(opc) == ILTY_STORE || IL_TYPE(opc) == ILTY_LOAD) &&
            ...
            case IL_LDA:
              ...
              if (rewr_new_nme)
                rewr_new_nme =
                    build_sym_nme((SPTR)basesym_of(rewr_new_nme), // ???
...
simplified_cmp_ili(int cmp_ili)
{
  ...
  while ((
      opc = (ILI_OP)opc_comp_zero_one(cmp_ili, &invert_cc, &label_op))) { // ???
