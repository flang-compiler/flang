...
void ENTF90(MATMUL_CPLX16,
            matmul_cplx16mxv_t)(char *dest_addr, char *s1_addr, char *s2_addr,
                                ...
                                F90_Desc *s1_desc, F90_Desc *s2_desc)
{
  ...
  if ((s1_d1_sstride == 1) && (s2_d1_sstride == 1) && (d_d1_sstride == 1) &&
    ...
    if (s2_rank == 1) {
      F90_MATMUL(cplx16_str1_mxv_t)( dest_base + d_d1_soffset*d_d1_lstride +
  ...
  /* transpose s1 */
  {
    ...
    for (k = 0; k < k_extent; k++) {
      ...
      for (m = 0; m < m_extent; m++) {
        s1_p = s1_base + s1_d1_offset;
        s1_d1_offset += s1_m_delta;
        s2_p = s2_base + s2_d2_base;
        rslt_tmp.r = 0;
        rslt_tmp.i = 0;
        for (n = 0; n < n_extent; n++) {
          rslt_tmp.r += s1_p->r * s2_p->r - s1_p->i * s2_p->i;
          rslt_tmp.i += s1_p->r * s2_p->i + s1_p->i * s2_p->r;
