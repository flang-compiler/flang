...
/**
 * \file
 * \brief pghpf.h - message passing related definitions
 */
...
/* Assume extent of gen_block == extent of processor arrangement ...
 * Used when building gen_block template internally where the extent of the
 * gen_block copy has already been checked... (it's also easier to pass the
 * gb args internally this way using a general template/qopy_in routine call)...
 * This way, we do not need an extra "extent" argument for template/qopy_in.
 * This should only be used within the run-time.
 */
#define __ASSUMED_GB_EXTENT 0x08000000
/* Used internally when calling ENTFTN(template) (and recognized in ENTFTN(qopy).
 * Tells ENTFTN(template) and ENTFTN(qopy_in) that the collapse dim has a dummy
 * paxis and dummy dstfmt arg being passed in (normally collapse dims have no
 * paxis and dstfmt arg).
 * Makes passing collapse dims easier to these routines when calling within
 * the run-time...
 */
...
struct F90_DescDim {/* descriptor dimension info */
  ...
  __INT_T lbound;  /* (1) global lower bound */
  __INT_T extent;  /* (2) global array extent */
  __INT_T sstride; /* (3) section index stride on array */
  __INT_T soffset; /* (4) section offset onto array */
  __INT_T lstride; /* (5) local index multiplier */
  __INT_T ubound;  /* (6) global lower bound (temporary) */
};

/* NOTE: The fields minus F90_DescDim below should remain consistent
 * with the OBJECT_DESC structure in type.h in terms of length and type.
 */
struct F90_Desc {/* file descriptor header */

  __INT_T tag;               /* (1) __DESC */
  ...
  __INT_T lbase;             /* (8) linearized index base offset */
  POINT(__INT_T, gbase);     /*     address of first element of section */
  POINT(DIST_Desc, dist_desc); /*   base of 'DIST' descriptor; the corresponding
                                *   word in the F90 object descriptor holds
                                *   the pointer to the type descriptor */
  F90_DescDim dim[MAXDIMS];  /*     first F90 dimension (place holder) */
};
...
/* same as above without gsize argument */
