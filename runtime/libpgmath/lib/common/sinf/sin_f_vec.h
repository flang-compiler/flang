...
__reduction_slowpath(vfloat const a, vmask *h)
{
    ...
    ue.v = e; uhi.v = hi; ulo.v = lo; ull.v = ll; us.v = s;
...
vfloat static INLINE
__sin_f_vec(vfloat const x)
{

    vfloat a, k, r;
    vint2 p, h;

    k = vfma_vf_vf_vf_vf(x, vcast_vf_f(_1_OVER_PI_F), vcast_vf_f(12582912.0f));
    h = vsll_vi2_vi2_i((vint2)k, 31);
    k = vsub_vf_vf_vf(k, vcast_vf_f(12582912.0f));

    a = vfma_vf_vf_vf_vf(k, vcast_vf_f(-PI_HI_F), x);
    a = vfma_vf_vf_vf_vf(k, vcast_vf_f(-PI_MI_F), a);
    a = vfma_vf_vf_vf_vf(k, vcast_vf_f(-PI_LO_F), a);

    r = __sin_kernel(a, h);

    p = vand_vi2_vi2_vi2((vint2)x, vcast_vi2_i(0x7fffffff));
    vopmask m = vgt_vo_vi2_vi2(p, (vint2)vcast_vf_f(THRESHOLD_F));
