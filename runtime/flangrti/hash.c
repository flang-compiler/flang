...
/*
   Open adressing, quadratically probed hash tables.
...
/* A hashmap adds a table of data pointers after the hashset table. */
struct hashmap_ {
  struct hashset_ set;
};
...
/* Search for key, return index that terminated the search. */
static unsigned
...
#if DEBUG
  assert(hashset_lookup(h, key) == NULL);
#endif
...
#if DEBUG
  assert(hashmap_lookup(h, key, NULL) == NULL);
#endif
