...
! one dimension out of two
	program p
	 ...
	 eopc=eoshift(eoshift(src,j,dim=2), 1, dim=1)	! eoshift, positive, constant
	 eopv=eoshift(eoshift(src,j,dim=2), j, dim=1)	! eoshift, positive, variable
	 eomc=eoshift(eoshift(src,j,dim=2),-1, dim=1)	! eoshift, negative, constant
	 eomv=eoshift(eoshift(src,j,dim=2), k, dim=1)	! eoshift, negative, variable
	  cpc= cshift(eoshift(src,j,dim=2), 1, dim=1)	!  cshift, positive, constant
	  cpv= cshift(eoshift(src,j,dim=2), j, dim=1)	!  cshift, positive, variable
	  cmc= cshift(eoshift(src,j,dim=2),-1, dim=1)	!  cshift, negative, constant
	  cmv= cshift(eoshift(src,j,dim=2), k, dim=1)	!  cshift, negative, variable
	 if( doprint )then
	  print 10, ' src',src
	  print 10, 'eopc',eopc
	  print 10, 'eopv',eopv
