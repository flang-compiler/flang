...
!
! test f2008 acos function taking a complex arguement
...
program p 
 ...
 use check_mod
  ...
  valuecf = CMPLX(-3.14, -.9999)
  valuecd = CMPLX(-3.14_8, -.9999_8)
  do i =  0,N-1 
    cf_src1(i+1) = valuecf + CMPLX(i*.697, i*.22)
    cd_src1(i+1) = valuecd + CMPLX(i*.697_8, i*.22_8)
  enddo

  cf_rslt = acos(cf_src1)
  cd_rslt = acos(cd_src1)
