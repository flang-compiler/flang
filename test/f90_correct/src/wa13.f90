...
! not constants, so the size must be determined at run time.

subroutine foo(lower, upper)
  implicit none
  integer :: lower, upper
  ...
  d = reshape((/ (a(sum(b(:j-1))+1:sum(b(:j))),  &
       (0, i = 1, k-b(j)), j = lower, upper) /), (/ k, upper /))
...
!  print *, results
  call check(results, expect, upper*k)
end subroutine foo
