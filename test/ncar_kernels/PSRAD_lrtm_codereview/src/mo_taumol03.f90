...
MODULE mo_taumol03
    ...
    CONTAINS
    SUBROUTINE taumol03_lwr(ncol, startCol, laytrop, nlayers,         &
        ...
        integer, intent(in) :: startCol              !starting index column
        integer, intent(in) :: laytrop               ! number of layers forwer atmosphere kernel
        integer, intent(in) :: nlayers               ! total number of layers
        real(kind=wp), intent(in), dimension(startCol:ncol,0:1,nlayers) :: rat_h2oco2,fac0,fac1    ! not sure of ncol depend
        real(kind=wp), intent(in), dimension(startCol:ncol,nlayers) :: colco2,colh2o,coln2o,coldry ! these appear to be gas concentrations

        real(kind=wp), intent(in), dimension(startCol:ncol,nlayers) :: selffac,selffrac            ! not sure of ncol depend
        real(kind=wp), intent(in), dimension(startCol:ncol,nlayers) :: forfac,forfrac              ! not sure of ncol depend
        real(kind=wp), intent(in), dimension(startCol:ncol,nlayers) :: minorfrac                   ! not sure of ncol depend

        ! Look up tables and related lookup indices
        ! I assume all lookup indices depend on 3D position
        ! =================================================

        integer, intent(in) :: jp(startCol:ncol,nlayers)      ! I assume jp depends on ncol
        integer, intent(in) :: jt(startCol:ncol,0:1,nlayers)  ! likewise for jt
        integer, intent(in) :: ig                    ! ig indexes into lookup tables
        integer, intent(in) :: indself(startCol:ncol,nlayers) ! self index array
        integer, intent(in) :: indfor(startCol:ncol,nlayers)  ! for index array
        integer, intent(in) :: indminor(startCol:ncol,nlayers) ! ka_mn2o index array
        real(kind=wp), intent(out), dimension(startCol:ncol,nlayers) :: taug  ! kernel result
        real(kind=wp), intent(out), dimension(startCol:ncol,nlayers) :: fracs ! kernel result
        ...
        integer, dimension(startCol:ncol)    :: caseType
        real(kind=wp), dimension(startCol:ncol) :: p, p4, fs
        real(kind=wp), dimension(startCol:ncol) :: fmn2o,fmn2omf
        real(kind=wp), dimension(startCol:ncol) :: fpl
        real(kind=wp), dimension(startCol:ncol) :: specmult, speccomb, specparm
        real(kind=wp), dimension(startCol:ncol) :: specmult_mn2o, speccomb_mn2o,specparm_mn2o
        real(kind=wp), dimension(startCol:ncol) :: specmult_planck, speccomb_planck,specparm_planck
        real(kind=wp), dimension(startCol:ncol) :: n2om1,n2om2,absn2o,adjcoln2o,adjfac,chi_n2o,ratn2o
        real(kind=wp), dimension(startCol:ncol,0:1) :: tau_major
        real(kind=wp), dimension(startCol:ncol) :: taufor,tauself
        integer, dimension(startCol:ncol) :: js, ind0, ind00, ind01, ind02, jmn2o, jpl
        ...
        ! ======================
            ...
            DO i=0,1         ! loop over specparm types
                ...
                ! =============================
                !dir$ SIMD
                DO icol=startCol,ncol  ! Vectorizes with dir - 14.0.2
            ...
            !dir$ SIMD
            do icol=startCol,ncol  ! vectorized with dir 14.0.2
    ...
    SUBROUTINE taumol03_upr(ncol, startCol, laytrop, nlayers,                     &
                rat_h2oco2, colco2, colh2o, coln2o, coldry, &
                fac0, fac1, minorfrac, &
                forfac,forfrac,        &
                jp, jt, ig,        &
        ...
        integer, intent(in) :: ncol                  ! number of simd columns
        integer, intent(in) :: startCol              ! starting index for iterations in order to support parallelization across architectures
        ...
        real(kind=wp), dimension(startCol:ncol) :: fs
        real(kind=wp), dimension(startCol:ncol) :: fmn2o,fmn2omf
        real(kind=wp), dimension(startCol:ncol) :: fpl
        real(kind=wp), dimension(startCol:ncol) :: specmult, speccomb, specparm
        real(kind=wp), dimension(startCol:ncol) :: specmult_mn2o, speccomb_mn2o, specparm_mn2o
        ...
        real(kind=wp), dimension(startCol:ncol,0:1) :: tau_major
        real(kind=wp), dimension(startCol:ncol) :: taufor,tauself
        integer, dimension(startCol:ncol) :: js, ind0, jmn2o, jpl
        ...
        DO lay = laytrop+1, nlayers
            ...
            !dir$ SIMD
            DO icol=startCol,ncol ! loop vectorized with directive 14.0.2
                chi_n2o(icol) = coln2o(icol,lay)/coldry(icol,lay)
                ratn2o(icol) = 1.e20*chi_n2o(icol)/chi_mls(4,jp(icol,lay)+1)
            END DO

            DO icol=startCol,ncol ! Loop vectorized as is 14.0.2
                IF (ratn2o(icol) .GT. 1.5_wp) THEN
                    adjfac(icol) = 0.5_wp+(ratn2o(icol)-0.5_wp)**0.65_wp
                    adjcoln2o(icol) = adjfac(icol)*chi_mls(4,jp(icol,lay)+1)*coldry(icol,lay)*1.e-20_wp
                ELSE
            ...
            !dir$ SIMD
            DO icol=startCol,ncol ! loop vectorizes with directive 14.0.2
